...
<?endif;?>
	...
	$bShowVideoTab = (!empty($arResult['VIDEO']) || !empty($arResult['VIDEO_IFRAME']));
			...
			<div class="tab-content">
				...
				<?if($bShowVideoTab):?>
					<div class="tab-pane <?=(!($iTab++) ? 'active' : '')?>" id="video">
						...
						<div class="row video">
							<?
							if($arResult['VIDEO'] && $arResult['VIDEO_IFRAME'])
								$count = 2;
							elseif($arResult['VIDEO'])
								$count = count($arResult['VIDEO']);
							elseif($arResult['VIDEO_IFRAME'])
								$count = count($arResult['VIDEO_IFRAME']);
							?>
							<?if($arResult['VIDEO']):?>
								...
								<?endforeach;?>
							<?endif;?>
							<?if($arResult['VIDEO_IFRAME']):?>
								<?foreach($arResult['VIDEO_IFRAME'] as $i => $video):?>
									<div class="col-md-<?=($count > 1 ? 6 : 12);?> item">
										<div class="video_body">
